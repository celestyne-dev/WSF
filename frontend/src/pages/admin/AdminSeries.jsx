...
import { fetchSeriesAdmin } from '../../api/taxonomies'
...
export default function AdminSeries() {
  ...
  useEffect(() => {
    let active = true
    setError(null)
    fetchSeriesAdmin({ status, q: query, per_page: 100 })
      .then((res) => active && setRows(res.items))
      .catch(() => active && setError('Something went wrong loading series. Please try again.'))
    return () => {
      active = false
    }
  }, [status, query])
  ...
  return (
    <div>
      <AdminPageHeader
        title="Series"
        description="Ongoing editorial series that group related Articles."
        actions={
          <Link to="/admin/taxonomy/series/new" className="btn-primary !px-4 !py-2 text-xs">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 border border-taupe-300 bg-white px-3 py-2">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search series…" className="w-56 text-sm focus:outline-none" />
        </div>
      ...
      {!error && rows !== undefined && (
        rows.length ? (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-3">
                        {row.status === 'published' && (
                          <a href={`/series/${row.slug}`} target="_blank" rel="noreferrer" aria-label="View live" className="text-charcoal-600 hover:text-burgundy-600">
                            <ExternalLink size={15} />
                          </a>
                        )}
                        <Link to={`/admin/taxonomy/series/${row.id}`} aria-label="Edit" className="text-charcoal-600 hover:text-burgundy-600">
                          <Pencil size={15} />
                        </Link>
                      </div>
                    </td>
          ...
          <EmptyState title="No series match those filters yet" description="Create a new series or broaden your search." />
        )
      )}
    </div>
  )
